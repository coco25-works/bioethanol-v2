# Program Overview

## Objective
Build a structured internal reference system covering global bio-based fuel regulation, demand, supply potential, and strategy, with a specific decision lens for a Southeast Asia-based producer.

## Primary Audience
The primary user is a commercial / business development team that needs to screen opportunities, compare target markets, understand buyer pull, and decide which pathway-market combinations are worth pursuing.

Secondary users:
- leadership and investors who need portfolio-level priorities and risk framing;
- operating and compliance teams who need to understand capability gaps and access requirements.

## Deliverable Model
The work product is not a single report. It is a layered document system:
- source register;
- country-level regulatory references;
- biofuel-pathway references;
- market analysis;
- strategy outputs;
- appendices and cross-reference tools.

## Scope
### In scope
- Biodiesel and fatty acid methyl ester (FAME)
- Renewable diesel / hydrotreated vegetable oil (HVO) where policy-relevant
- Bioethanol
- Biomethanol
- Biogas and renewable natural gas (RNG)
- Sustainable aviation fuel (SAF) relevance where linked to the pathways above

### Jurisdiction tiers
- Deep coverage: EU, Germany, Netherlands, France, US federal, California, UK, Japan
- Medium coverage: Brazil, Singapore
- Context coverage: China, Indonesia, Malaysia, Thailand

## Required Analytical Views
Every major topic should ultimately support these questions:
- What regulation creates demand?
- Which fuels and feedstocks are eligible, capped, restricted, or conditional?
- Which counterparties are actually obligated or incentivized to buy?
- Where is supply structurally short?
- Where can a Southeast Asia-based producer compete?
- What certification, traceability, and chain-of-custody conditions gate access?
- What commercial decision does this file support?
- What would change the conclusion or make the team stop pursuing the opportunity?

## Operating Principles
- Primary law first for legal meaning.
- Official guidance second for implementation.
- Official data first for market facts.
- Strategy only after validated source and analysis layers exist.
- Use `eligible / restricted / capped / conditional` language instead of collapsing everything into `allowed / banned`.
- Separate legal fact, agency interpretation, market observation, and strategy inference in every downstream document.
- Make buyer logic, decision criteria, and stop conditions explicit before allowing a strategy recommendation.
- Treat evidence confidence as part of the output, not an internal note.

## Program Outputs
- `00_program_requirements/`: method, scope, taxonomy, and frameworks
- `01_source_register/`: per-jurisdiction source logs
- `02_country_reference/`: per-jurisdiction reference files
- `03_biofuel_reference/`: per-pathway synthesis files
- `04_market_analysis/`: cross-market and demand/supply analysis
- `05_strategy_outputs/`: separate strategy documents
- `06_appendices/`: glossary, citation index, and gap log
